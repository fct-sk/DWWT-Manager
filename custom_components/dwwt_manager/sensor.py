...
SENSORS = (
    ...
    DwwtSensorDescription(key="pump_starts_total", translation_key="pump_starts_total", state_class=SensorStateClass.TOTAL, value_fn=lambda m: m.state.pump_starts_total),
    ...
    *tuple(DwwtSensorDescription(key=f"starts_{hours}h", translation_key=f"starts_{hours}h", state_class=SensorStateClass.MEASUREMENT, entity_registry_enabled_default=hours != 6, value_fn=lambda m, h=hours: m.starts(h)) for hours in (1, 6, 24)),
